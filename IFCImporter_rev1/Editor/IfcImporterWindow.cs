...
namespace IFCImporter.Editor
{
    ...
    public class IfcImporterWindow : EditorWindow
    {
        ...
        // Robotics options
        private bool _showRoboticsOptions = true;
        private bool _enableRoboticsMode = true;
        private bool _assignLayers = true;
        private bool _simplifyColliders = true;
        private bool _prepareNavMesh = false;
        private bool _createSemanticSpaces = true;
        private bool _createOpenings = true;

        // Incremental import
        private bool _incrementalImport = false;
        ...
        private void DrawImportOptions()
        {
            ...
            EditorGUILayout.LabelField("Delta / Incremental Import", EditorStyles.boldLabel);
            _incrementalImport = EditorGUILayout.Toggle(
                new GUIContent("Incremental Import",
                    "Only re-import elements that changed since the last import. " +
                    "Compares element fingerprints stored in a manifest file."),
                _incrementalImport);
            if (_incrementalImport)
            {
                EditorGUILayout.HelpBox(
                    "When enabled, the importer will compare element content hashes " +
                    "against the previous import manifest. Unchanged elements are " +
                    "skipped, modified elements are updated in-place, and deleted " +
        ...
        private void DrawRoboticsOptions()
        {
            _showRoboticsOptions = EditorGUILayout.Foldout(_showRoboticsOptions, "Robotics Options", true);

            if (_showRoboticsOptions)
            {
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);

                _enableRoboticsMode = EditorGUILayout.Toggle(
                    new GUIContent("Enable Robotics Mode",
                        "Master toggle: adds IfcRegistry, IfcRelations, IfcPassage components and wires IFC relationships."),
                    _enableRoboticsMode);

                using (new EditorGUI.DisabledScope(!_enableRoboticsMode))
                {
                    _assignLayers = EditorGUILayout.Toggle(
                        new GUIContent("Assign Robotics Layers",
                            "Set GameObjects to Walkable / Obstacle / Door / Opening / Space layers."),
                        _assignLayers);

                    _simplifyColliders = EditorGUILayout.Toggle(
                        new GUIContent("Simplify Colliders",
                            "Use BoxColliders for walls, slabs, columns, doors; MeshColliders for complex shapes."),
                        _simplifyColliders);

                    _prepareNavMesh = EditorGUILayout.Toggle(
                        new GUIContent("Prepare NavMesh",
                            "Mark walkable surfaces for Unity Navigation bake (requires AI Navigation package)."),
                        _prepareNavMesh);

                    _createSemanticSpaces = EditorGUILayout.Toggle(
                        new GUIContent("Create Semantic Spaces",
                            "Create GameObjects for IfcSpace even without geometry."),
                        _createSemanticSpaces);

                    _createOpenings = EditorGUILayout.Toggle(
                        new GUIContent("Create Openings",
                            "Import IfcOpeningElement entities for clearance analysis."),
                        _createOpenings);
                }
        ...
        private void StartImport()
        {
            ...
            var options = new IfcImportOptions
            {
                ...
                // Robotics
                EnableRoboticsMode = _enableRoboticsMode,
                AssignLayers = _enableRoboticsMode && _assignLayers,
                SimplifyColliders = _enableRoboticsMode && _simplifyColliders,
                PrepareNavMesh = _enableRoboticsMode && _prepareNavMesh,
                CreateSemanticSpaces = _enableRoboticsMode && _createSemanticSpaces,
                CreateOpenings = _enableRoboticsMode && _createOpenings,
                SourceFileReference = _ifcFilePath,
                IncrementalImport = _incrementalImport
            };
            ...
            try
            {
                ...
                // ─── Incremental import path ───
                if (_incrementalImport)
                {
                    Log("Attempting incremental import (delta-update)...");
                    var delta = processor.TryIncrementalImport(_ifcFilePath, UpdateProgress);

                    if (delta != null)
                    {
                        // Incremental succeeded
                        Log($"Incremental import done: {delta.GetSummary()}");
                        ...
                        Log($"+{delta.AddedIds.Count} added, ~{delta.ModifiedIds.Count} modified, -{delta.RemovedIds.Count} removed");
                        Log($"={delta.UnchangedIds.Count} unchanged (skipped)");
                        Log("═══════════════════════════════════════");
                    ...
                    Log("Incremental import unavailable — falling back to full import...");
                }
