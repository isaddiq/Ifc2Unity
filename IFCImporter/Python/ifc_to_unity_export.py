...
# IFC types to export (physical building elements) - expanded list
ALLOWED_IFC_TYPES = [
    "IfcWall", "IfcWallStandardCase", "IfcCurtainWall",
    "IfcSlab", "IfcRoof", "IfcStair", "IfcStairFlight", "IfcRamp", "IfcRampFlight",
    "IfcBeam", "IfcBeamStandardCase", "IfcColumn", "IfcColumnStandardCase",
    "IfcMember", "IfcMemberStandardCase",  # Includes balusters/spindles in railings
    "IfcDoor", "IfcDoorStandardCase", "IfcWindow", "IfcWindowStandardCase",
    "IfcPlate", "IfcPlateStandardCase", "IfcFooting", "IfcPile",
    "IfcRailing",  # Main railing
    ...
    "IfcCovering", "IfcCeiling",
    "IfcLightFixture", "IfcSanitaryTerminal",
    "IfcSpace", "IfcShadingDevice",
    ...
    "IfcTransportElement", "IfcGeographicElement",
    "IfcCivilElement", "IfcElementAssembly",
    "IfcChimney", "IfcBearing", "IfcDeepFoundation",
    "IfcCoil", "IfcCompressor", "IfcCondenser", "IfcCooledBeam", "IfcCoolingTower",
    ...
    "IfcFan", "IfcFilter", "IfcFireSuppressionTerminal", "IfcHeatExchanger",
    "IfcHumidifier", "IfcJunctionBox", "IfcLamp", "IfcMedicalDevice",
    "IfcMotorConnection", "IfcOutlet", "IfcProtectiveDevice", "IfcProtectiveDeviceTrippingUnit",
    "IfcPump", "IfcSolarDevice", "IfcSpaceHeater", "IfcStackTerminal",
    "IfcSwitchingDevice", "IfcTank", "IfcTransformer", "IfcTubeBundle",
    "IfcUnitaryControlElement", "IfcUnitaryEquipment", "IfcValve", "IfcWasteTerminal",
]

# IFC types to exclude (non-physical or handled separately)
EXCLUDED_IFC_TYPES = [
    "IfcOpeningElement",
    "IfcAnnotation",
    "IfcGrid",
...
def filter_products(ifc_file):
            ...
            for rel in element.IsDecomposedBy:
                if hasattr(rel, 'RelatedObjects'):
                    for child in rel.RelatedObjects:
                        if hasattr(child, 'Representation') and child.Representation:
    ...
    for product in all_products:
        ...
        excluded = False
        for exc in EXCLUDED_IFC_TYPES:
            if product.is_a(exc):
                excluded = True
                key = f"type_filter:{ifc_class}"
                stats['filtered_out'][key] = stats['filtered_out'].get(key, 0) + 1
                break
        
        if excluded:
            continue
            
        # Check if it has geometry representation
        if not product.Representation:
            stats['filtered_out']['no_representation'] = stats['filtered_out'].get('no_representation', 0) + 1
            continue
        
        # Check if it's an allowed type
        is_allowed = False
        for allowed in ALLOWED_IFC_TYPES:
            if product.is_a(allowed):
                is_allowed = True
                break
        ...
        if is_allowed:
            ...
            for child in decomposed_children:
                child_class = child.is_a()
                # Check child is not excluded
            ...
            # Include ANY element that has a Body, Facetation, Brep, or SurfaceModel representation
            has_exportable_rep = False
            for rep in product.Representation.Representations:
                if rep.RepresentationIdentifier in ['Body', 'Facetation', 'Brep', 'SurfaceModel', 'SolidModel', 'Tessellation', 'AdvancedBrep', 'Clipping', 'CSG']:
                    has_exportable_rep = True
                    break
                # Also check for mapped representations
                if rep.RepresentationIdentifier == 'MappedRepresentation':
                    has_exportable_rep = True
                    break
                # Check items for any geometry
                if rep.Items:
                    for item in rep.Items:
                        if hasattr(item, 'is_a'):
                            item_type = item.is_a()
                            if any(geo_type in item_type for geo_type in ['Brep', 'Solid', 'Surface', 'Face', 'Shell', 'Tessellat', 'Triangulat', 'Polygon']):
                                has_exportable_rep = True
                                break
                    if has_exportable_rep:
                        break
                        
            if has_exportable_rep:
                add_element(product)
            else:
                key = f"no_body_rep:{ifc_class}"
                stats['filtered_out'][key] = stats['filtered_out'].get(key, 0) + 1
...
def write_metadata_csv(filepath, all_metadata):
    ...
    priority_cols = ['GlobalId', 'Name', 'IfcClass', 'TypeName', 'Project', 'Site', 
                     'Building', 'Storey', 'Space', 'Material', 'Color_R', 'Color_G', 
                     'Color_B', 'Color_A', 'Description', 'ObjectType', 'Tag', 'TypeId']
...
def export_spatial_hierarchy(ifc_file):
    """Export spatial hierarchy elements (Project, Site, Building, Storey)."""
    hierarchy_metadata = []
    ...
    for storey in ifc_file.by_type('IfcBuildingStorey'):
        storey_name = format_hierarchy_name('IfcBuildingStorey', storey.Name)
        hierarchy_metadata.append({
...
def export_ifc_to_unity(ifc_path, output_dir=None):
    ...
    print(f"Added {len(hierarchy_metadata)} spatial hierarchy records")
